...
def render_ui(target_df, is_facility=False):
    ...
    for _, row in target_df.iterrows():
        ...
        if raw_tel.startswith('*1'):
            full_tel = "022258" + raw_tel.replace('*1', '')
        elif raw_tel:
            full_tel = "023147" + raw_tel
        else:
            full_tel = ""

        hp_link = re.sub(r'[^0-9]', '', raw_hp)

        # 시설 탭 전용: 부서명 옆에 내선번호 노출
        dept_display = f"{dp} ({raw_tel})" if is_facility and raw_tel else dp

        # HTML 렌더링
        tel_btn = f'<a href="tel:{full_tel}" class="c-btn btn-tel">내선</a>' if full_tel else ''
        hp_btn = f'<a href="tel:{hp_link}" class="c-btn btn-hp">직통</a>' if hp_link else ''
            ...
            <div class="contact-item">
                <div class="info-group">
                    <div class="name-row">
                        ...
                        <span class="dept-text">{dept_display}</span>
                    </div>
                    {work_div}
                </div>
                <div class="btn-group">{tel_btn}{hp_btn}</div>
            </div>
        """, unsafe_allow_html=True)
