...
filtered_base = df[df.apply(lambda r: r.str.contains(q, case=False).any(), axis=1)] if q else df
...
def render_ui(target_df):
    ...
    for _, row in target_df.iterrows():
        ...
        # --- [노출 제어 로직] ---
        if is_real_security:
            # 보안팀: 번호 텍스트(hp_html)는 안 보여주지만, 버튼(m_btn_html)은 생성함
            ...
            # 그 외 부서: 번호 텍스트와 버튼 모두 노출 (내선 없으면 모바일 텍스트 노출)
